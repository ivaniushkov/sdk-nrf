...
/* Row layout shared by every generated <lib>_log_msg.h table (see
 * dragoon/mpsl/common_build_tools/log/collect_log_entries.py). Declared here, rather than
 * taken from one specific generated header, so this handler works with any subset of
 * contributors (MPSL, SDC, ...) enabled independently of each other.
 */
...
static const struct mpsl_log_msg_row *mpsl_log_msg_lookup(uint32_t id)
{
	...
	case MPSL_LOG_LIB_ID_SDC:
		return (const struct mpsl_log_msg_row *)
...
{
	const struct mpsl_log_msg_row *msg = mpsl_log_msg_lookup(entry->id);
